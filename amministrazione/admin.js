// Cascina Fontana — Admin dashboard (listings + updates)
(function () {
  ...
  function bootFail(msg) {
    bootLoader.innerHTML =
      ...
      `<p class="mt-3"><a href="/amministrazione/login.html" class="text-primary underline">Login</a></p>`;
  }
  ...
  async function boot() {
    ...
    if (error || !data.session) {
      window.location.href = '/amministrazione/login.html';
      return;
    }
    ...
    window.sb.auth.onAuthStateChange((event) => {
      if (event === 'SIGNED_OUT') window.location.href = '/amministrazione/login.html';
    });
  }
  ...
  const Listings = (() => {
    let editingId = null;
    let currentUrls = [];   // image URLs already uploaded / on record
    ...
    function onPick() {
      ...
      // Local preview for newly picked files (append visually)
      ...
      Promise.all(readers).then(previews => {
        ...
        // Thumbs show combined; removal acts only on currentUrls (indices before new)
        ...
        container.innerHTML = combined.map((u, i) => `
          <div class="relative group">
            ...
            ${i < currentUrls.length
              ...
              : `<span class="absolute -top-2 -right-2 px-1.5 py-0.5 rounded-full bg-primary text-dark-bg text-[9px] font-bold">NEW</span>`
            }
          </div>
        `).join('');
        container.querySelectorAll('[data-rm]').forEach(btn => {
